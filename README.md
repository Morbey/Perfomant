...
A key design principle of Performant is **summary-first reasoning**: MCP tools fetch and process raw data locally, and only a compact, structured summary is sent to the language model. This is done using a shared JSON summary pattern (see `docs/patterns/summary_schema.json`), which drastically reduces token usage and keeps the model focused on decisions, not raw data shovelling.
